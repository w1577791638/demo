...
?>
	...
	<div class="com-hd">
		<b><img src="/content/plugins/MyhkPlayer/style/logo.png" align="middle" style="margin-top:-10px"/> <a href="https://myhkw.cn" target="_blank">明月浩空音乐播放器</a></b>
		<?php
	...
	<form action="plugin.php?plugin=MyhkPlayer&action=setting" method="post">
		<table class="tb-set">
			<tr>
				...
				<td><span class="sel"><select name="xl"><option value="@" <?php if($config["xl"]=="@") echo "selected"; ?>>主站</option><option value="player" <?php if($config["xl"]=="player") echo "selected"; ?>>一线</option><option value="music" <?php if($config["xl"]=="music") echo "selected"; ?>>二线</option></select></span></td>
			</tr>
			<tr>
			...
			<tr>
				<td align="right"><b>播放器ID：</b><br />(填写<b>播放器控制面板</b>自己的播放器ID，免注册公用ID：demo)</td>
				<td><input type="text" class="txt" name="id" value="<?php echo $config["id"]; ?>" /></td>
			</tr>
			<tr>
				<td align="right"><b>后台地址：</b><br />(免费注册创建播放器后台管理地址)</td>
				<td><b><a href="https://player.lmih.cn" target="_blank">https://myhkw.cn</b></a></td>
			</tr>
			<tr>
				<td align="right"></td>
